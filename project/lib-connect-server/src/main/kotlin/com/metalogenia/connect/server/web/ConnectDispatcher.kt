...
class ConnectDispatcher(
    ...
    private fun handleConnectUnary(
        ...
        compressions.resolve(request.header(ConnectHeaders.CONTENT_ENCODING))?.let { body = it.decompress(body) }
    ...
    private fun handleConnectGet(
        ...
        compressions.resolve(request.queryParam("compression"))?.let { body = it.decompress(body) }
        val message = codec.deserialize(body, entry.requestPrototype)
    ...
    private fun handleEnveloped(
        ...
        val requestCompression = compressions.resolve(request.header(requestEncodingHeader))
    ...
    private fun decodeEnvelopedRequest(
        ...
        if (dataFrame.flags and Envelope.FLAG_COMPRESSED != 0) {
            bytes = (compression ?: throw ConnectException(ConnectCode.INVALID_ARGUMENT, "compressed frame without encoding"))
                .decompress(bytes)
        }
    ...
    private fun connectTimeoutMillis(request: ConnectHttpRequest): Long? =
        request.header(ConnectHeaders.TIMEOUT_MS)?.trim()?.toLongOrNull()
