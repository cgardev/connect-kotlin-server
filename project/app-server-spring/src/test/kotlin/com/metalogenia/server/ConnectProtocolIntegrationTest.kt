...
class ConnectProtocolIntegrationTest {
    ...
    private fun url(method: String) = "http://localhost:$port/connect.demo.v1.EchoService/$method"
    ...
    fun `idempotent unary over GET`() {
        ...
        assertEquals(200, response.statusCode())
        assertEquals("connect-rpc-kotlin-server", mapper.readTree(response.body()).get("name").asText())
    }
